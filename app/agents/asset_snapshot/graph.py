...
def build_asset_snapshot_graph(
    ...
    async def sector_context_tool_node_wrapper(state: AssetSnapshotState):
        return await sector_context_tool_node(state, sector_context_tool)

    async def company_peers_tool_node_wrapper(state: AssetSnapshotState):
        return await company_peers_tool_node(state, company_peers_tool)
    ...
    graph.add_node("sector_context_tool", sector_context_tool_node_wrapper)
    graph.add_node("company_peers_tool", company_peers_tool_node_wrapper)
    ...
    graph.add_edge("asset_profile_tool", "sector_context_tool")
    graph.add_edge("sector_context_tool", "company_peers_tool")
    graph.add_edge("company_peers_tool", "generate_snapshot")
